...
class CourtsController extends Controller
{
    ...
    public function get()
    {
        return DataTables::of(Court::query())
            ->addColumn('actions', function ($court) {
                return '<button type="button" class="btn btn-primary editCourtButton"
                        data-id="' . $court->id . '"
                        data-court_name="' . $court->court_name . '">Edit</button>
                        <button type="button" class="btn btn-danger deleteCourtButton"
                        data-id="' . $court->id . '">Delete</button>';
            })
            ->rawColumns(['actions'])
            ->make(true);
    }
    ...
    public function update(Request $request, Court $court)
    {
        ...
        $court->update($request->all())->where('id',$request->input('id'));
