    ...
    public function get()
    {
        return DataTables::of(Court::query())
            ->addColumn('actions', function ($court) {
                return '<button type="button" class="btn btn-primary btn-sm editCourtButton" data-id="' . $court->id . '" data-court_name="' . $court->court_name . '"><i class="fa fa-edit"></i> Edit</button>
                        <button type="button" class="btn btn-danger btn-sm deleteCourtButton" data-id="' . $court->id . '"><i class="fa fa-trash"></i> Delete</button>';
            })
            ->rawColumns(['actions'])
            ->make(true);
    }
