    ...
    public function getCaseDiaries(Request $request)
    {
        ...
        return DataTables::eloquent($caseDiariesQuery)
            ->addColumn('actions', function ($caseDiary) {
                return '<button type="button" class="btn btn-primary btn-sm editCaseDiaryButton" data-id="' . $caseDiary->id . '"><i class="fa fa-edit"></i></button>
                    <button type="button" class="btn btn-danger btn-sm deleteCaseDiaryButton" data-id="' . $caseDiary->id . '"><i class="fa fa-trash"></i></button>';
            })
            ->rawColumns(['actions'])
            ->make(true);
    }
